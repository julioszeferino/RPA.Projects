...
def _tagui_output():
    ...
    init_directory_output_file = os.path.join(
        _tagui_init_directory, 'rpa_python.txt')
...
def _tagui_delta(base_directory=None):
    ...
    if platform.system() in ['Linux', 'Darwin']:
        os.system('chmod -R 755 ' + base_directory + '/' +
                  'src' + '/' + 'tagui > /dev/null 2>&1')
        os.system('chmod -R 755 ' + base_directory + '/' +
                  'src' + '/' + 'end_processes > /dev/null 2>&1')
    ...
    delta_done_file.write(_py23_write(
...
def setup():
    ...
    if platform.system() == 'Darwin':
        ...
        if not os.path.isfile('/usr/local/opt/openssl/lib/libssl.1.0.0.dylib'):
            ...
            os.system('brew uninstall openssl > /dev/null 2>&1')
            os.system(
                'brew install https://github.com/tebelorg/Tump/releases/download/v1.0.0/openssl.rb > /dev/null 2>&1')

            # if it is still missing, attempt again by first installing homebrew
            if not os.path.isfile('/usr/local/opt/openssl/lib/libssl.1.0.0.dylib'):
                print('')
                print(
                    '[RPA][INFO] - now installing OpenSSL dependency using Homebrew')
                print(
                    '[RPA][INFO] - you may be prompted for login password to continue')
                print('')
                os.system(
                ...
                if not os.path.isfile('/usr/local/opt/openssl/lib/libssl.1.0.0.dylib'):
                    print(
                        '[RPA][INFO] - OpenSSL was not able to be installed automatically')
                    print(
                        '[RPA][INFO] - run below commands in your terminal to install manually')
                    print(
                        '[RPA][INFO] - after that, TagUI ready for use in your Python environment')
                    print('')
                    print(
                    ...
                    print('')
                    print(
                        '[RPA][INFO] - if there is an issue running brew command, check the solution below')
                    print(
                        '[RPA][INFO] - https://github.com/kelaberetiv/TagUI/issues/86#issuecomment-532466727')
                    print('')
                    return False
    ...
    try:
        ...
        while True:
            ...
            if 'LIVE MODE - type done to quit' in tagui_out:
                ...
                _tagui_write(
                    'echo "[RPA][' + str(_tagui_id) + '] - listening for inputs"\n')
...
def update():
    ...
    v_back_marker = "'"
    rpa_python_py = rpa_python_py[rpa_python_py.find(
        v_front_marker) + len(v_front_marker):]
...
"""
    ...
    try:
        ...
        print(
            '[RPA][INFO] - python update.py to update RPA for Python to version ' + rpa_python_py)
        return True
        ...
        if not debug():
            ...
            if os.path.isfile(os.path.join(_tagui_init_directory, 'rpa_python.log')):
                os.remove(os.path.join(
                    _tagui_init_directory, 'rpa_python.log'))
    ...
    if test_coordinate1 is not None and isinstance(test_coordinate1, int):
        if test_coordinate2 is not None and isinstance(test_coordinate2, int):
            if test_coordinate3 is not None and isinstance(test_coordinate3, int):
                element_identifier = coord(
                    element_identifier, test_coordinate1) + '-'
                element_identifier = element_identifier + \
                    coord(test_coordinate2, test_coordinate3)
        ...
        if test_coord1 is not None and isinstance(test_coord1, int):
            if test_coord2 is not None and isinstance(test_coord2, int):
                if test_coord3 is not None and test_coord3 != '':
                    element_identifier = coord(
                        element_identifier, filename_to_save) + '-'
                    element_identifier = element_identifier + \
                        coord(test_coord1, test_coord2)
...
def frame(main_frame=None, sub_frame=None):
    ...
    frame_identifier = '(//frame|//iframe)[@name="' + \
        main_frame + '" or @id="' + main_frame + '"]'
    if not exist(frame_identifier):
        print(
            '[RPA][ERROR] - cannot find frame with @name or @id as \'' + main_frame + '\'')
        return False

    send('js new_context = "mainframe_context"')
    send('js frame_xpath = \'(//frame|//iframe)[@name="' +
         main_frame + '" or @id="' + main_frame + '"]\'')
    ...
    if sub_frame is not None and sub_frame != '':
        frame_identifier = '(//frame|//iframe)[@name="' + \
            sub_frame + '" or @id="' + sub_frame + '"]'
        if not exist(frame_identifier):
            print(
                '[RPA][ERROR] - cannot find sub frame with @name or @id as \'' + sub_frame + '\'')
            return False
